...
- DATABASE_URL - путь к базе данных в виде унифицированного идентификатора ресурса (URI): _postgres://{user}:{password}@{hostname}:{port}/{database-name}_
...
- __Linux (Ubuntu):__
  ...
  - Вывести имеющиеся
    ...
    printenv
    ```
  - задать для пользователя через редактор nano:
    ```
    nano $HOME/.bashrc
    ```
  - задать для системы через редактор nano:
    ```
...
Остановить сервис можно сочетанием клавиш __CTRL + C__
</details>
<details>
